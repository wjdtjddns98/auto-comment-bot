...
200 `{ url }` — FE 는 "Threads로 연결" 버튼에서 이 URL 로 새 창/이동시킨다. 동의 완료 시
`https://nutti.co.kr/threads-callback.html?code=...` 로 리다이렉트되고, 그 페이지가 코드를
표시한다(수동 운반). 서버 OAuth 설정 미비 시 503.

### `POST /api/sns-accounts/threads-oauth` (CSRF 필수) — 코드로 계정 연동
Req `{ code, display_name? }` → 201 `{ id, user_id, platform, display_name, status, token_expires_at }`.
→ 서버가 코드→단기→**장기(60일) 토큰** 교환 후 암호화 저장. `token_expires_at` 이 실제
  만료 시각으로 채워진다(수동 등록과의 차이). `platform_username` 도 연동 시점에 확보.
→ **같은 username 재연동이면 새 계정을 만들지 않고 그 계정의 자격증명을 교체**(계정 id
  보존 — 소스 config 참조 유지) + `status=active` 복구. `display_name` 은 주면 갱신.
→ 코드 무효/만료/재사용 400(고정 메시지, echo 없음) · OAuth 설정 미비 503 ·
  암호화 키 미설정 503 · body 에 그 외 키 422(extra 금지).
