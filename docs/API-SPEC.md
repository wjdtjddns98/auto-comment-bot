...
> 확정 2026-08-12(이슈 #102). FE 제안 계약 그대로이며 **삭제만 가드가 붙는다**(아래).
> 스키마 변경 없음 — 마이그레이션 불필요.

### `GET /api/users` → 200 `[{ id, email, role, created_at }]` (id 오름차순)
`password_hash` 는 전용 read 스키마라 구조적으로 실리지 않는다.

### `POST /api/users` (CSRF 필수)
Req `{ email, password, role: "admin|reviewer" }` → 201 `{ id, email, role, created_at }`.
- `password` **8자 이상**(상한 200), `email` 3~255자·`@` 포함·공백 불가(앞뒤 공백은 서버가 제거).
  이메일 정규식 검증은 하지 않는다 — 로그인이 평문 정확 대조라, 여기서만 엄격하면
  "만들 수는 있는데 로그인이 안 되는" 계정이 생긴다.
- 409: 이미 등록된 이메일.

### `PATCH /api/users/{id}` (CSRF 필수)
Req `{ role }` → 200 `{ id, email, role, created_at }`.
- **409: 마지막 admin 의 강등.** 아무도 관리 화면에 못 들어가는 상태를 막는다.
- 자기 자신의 강등은 다른 admin 이 있으면 허용. 역할은 매 요청 DB 에서 읽으므로 **즉시 적용**된다
  (재로그인 불필요).

### `DELETE /api/users/{id}` → 204 (CSRF 필수)
**삭제는 "아무것도 하지 않은 계정" 에만 열린다.** 아래 중 하나라도 걸리면 **409 + 이유**:

| 조건 | 이유 |
|---|---|
| 자기 자신 | 실수 방지(FE 도 막지만 서버가 최종) |
| 마지막 admin | 영구 잠김 방지 |
| 소유한 소스·키워드·템플릿·SNS 계정이 있음 | `user_id` 가 **ON DELETE CASCADE** — 지우면 **함께 사라진다**(수집 중단) |
| 승인/처리 이력(`reply_actions`)이 있음 | 감사 로그 보존(FK RESTRICT). 대신 역할을 reviewer 로 낮춰 사용 중단 |
| `verify_pending` 매칭의 승인자 | 조정 잡이 승계 이력을 못 써 그 매칭이 영구히 갇힌다 |

> 즉 **퇴사자 계정 회수 용도로는 쓸 수 없다** — 그 용도는 소프트삭제(비활성화 컬럼)가
> 필요하고 별건이다. 지금은 잘못 만든 계정 회수용으로 보면 된다.
...
> - `naver_cafe`: `{ "query": "검색어(1~100자)" }` — 네이버 카페글 검색 API 로 **공개 카페의
>   공개 게시글**을 수집(읽기 전용, `can_write=false` → 승인은 클립보드 복사 경로).
>   자격증명은 앱 단위라 config 가 아니라 서버 `.env`(`NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`)
>   로 주입한다 — 미설정이면 등록은 되지만 수집이 실패하고 `last_error` 에 사유가 남는다.
>   FE 소스 폼에 naver_cafe 타입 추가 필요([FE 공유]).
>
> **naver_cafe 소스의 표시상 주의(FE 계약)**: 카페글 검색 응답에는 게시 시각과 글쓴이가
> 없다. 따라서 이 소스의 `matched_posts` 는 `published_at = null` 이고, `author` 에는
> 글쓴이 대신 **카페 이름**이 들어간다(출처 표시용, 의도된 동작). 날짜 정렬·작성자 링크를
> 거는 화면은 이 소스에서 두 값이 비거나 다른 의미임을 전제해야 한다.
...
>
> **소스 개수 상한(운영)**: 한도 25,000회/일이 **클라이언트 ID 합산**이라 소스 수가 곧
> 한도다 — 300초 주기면 소스당 288회/일로 **약 86개**, 하한인 60초 주기면 1,440회/일로
> **약 17개**가 끝이다. `poll-now` 수동 검색과 같은 키를 쓰는 다른 네이버 검색 API 사용분은
> 여기에 더해진다. 서버는 아직 개수를 강제하지 않는다(후속 R19) — 등록 시 사람이 지킬 것.
> 한도에 걸리면 어댑터가 **모든 naver_cafe 소스를 함께** 쿨다운시킨다(소스별 backoff 만으로는
> 이미 고갈된 한도를 계속 두드리게 되므로).
>
